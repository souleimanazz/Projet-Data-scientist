...
def find_numbers(list)
    count = 0
    list.each do |item|
        if item.count('1234567890')>0
            count+=1
        end
    end
    puts "Q> Combien d'handle contiennent un numéro ?"
    print "R> "
    puts count
end
...
def find_aude(list)
    ...
    list.each do |item|
        ...
        while i<item.length-3
            if item.downcase[i]=='a' && item.downcase[i+1]=='u'&& item.downcase[i+2] == 'd' && item.downcase[i+3] == 'e'
                count +=1
                i=item.length
            end
            i+=1
    ...
    puts "Q> Combien d'handle contiennent les 4 lettres du prénom 'Aude' à la suite (sans prendre en compte les majuscules) ? "
    print "R> "
    puts count
end

def combien_maj(list)
    puts "Q> Combien commencent par une majuscule (première lettre juste après le @) ?"
    print "R> "
    puts list.count{|item| item.match "@[A-Z]"}
end


def au_moins_maj(list)
...
def sort_list(list)
    sorted = list.sort_by {|word| word.downcase}
...
def perform(list)
    ...
    combien_maj(list)
    au_moins_maj(list)
    combien_underscore(list)
    sort_list(list)
    handle_plus_courts(list)
    epenser(list)
end
perform(logins)
